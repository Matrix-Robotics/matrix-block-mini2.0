...
Blockly.ScratchBlocks.ProcedureUtils.createArgumentReporter_ = function (
  ...
  if (argumentType == 's') {
    var blockType = 'argument_reporter_string_only';
...
Blockly.ScratchBlocks.ProcedureUtils.checkOldTypeMatches_ = function (oldBlock,
  ...
  if ((type == 's') && oldBlock.type == 'argument_reporter_string_only') {
    return true;
  }
  if ((type == 'n') && oldBlock.type == 'argument_reporter_string_number') {
    return true;
  }
...
Blockly.ScratchBlocks.ProcedureUtils.createArgumentEditor_ = function (
  ...
  try {
    if (argumentType == 's') {
      var newBlock = this.workspace.newBlock('argument_editor_string_only');
    } else if (argumentType == 'n') {
  ...
  for (var i = 0; i < this.inputList.length; i++) {
    ...
    if (input.type == Blockly.DUMMY_INPUT) {
      ...
      if (target.type == 'argument_editor_boolean') {
        this.procCode_ += '%b';
      } else if (target.type == 'argument_editor_string_only') {
        this.procCode_ += '%s';
      } else {
        this.procCode_ += '%n';
      }
...
Blockly.ScratchBlocks.ProcedureUtils.addLabelExternal = function () {
  ...
  this.procCode_ = this.procCode_ + ' label';
  this.updateDisplay_();
...
/**
 * Externally-visible function to add a string only argument to the procedure
 * declaration.
 * @public
 */
Blockly.ScratchBlocks.ProcedureUtils.addStringOnlyExternal = function () {
  Blockly.WidgetDiv.hide(true);
...
Blockly.ScratchBlocks.ProcedureUtils.updateArgumentReporterNames_ = function (prevArgIds, prevDisplayNames) {
  ...
  for (var i = 0; i < allBlocks.length; i++) {
    ...
    if ((block.type === 'argument_reporter_string_number' ||
      block.type === 'argument_reporter_string_only' ||
...
Blockly.Blocks['procedures_declaration'] = {
  ...
  addStringOnlyExternal: Blockly.ScratchBlocks.ProcedureUtils.addStringOnlyExternal,
...
Blockly.Blocks['argument_reporter_string_only'] = {
  init: function () {
    this.jsonInit({
      "message0": " %1",
      "args0": [
...
Blockly.Blocks['argument_editor_string_only'] = {
  init: function () {
    this.jsonInit({
      "message0": " %1",
      "args0": [
